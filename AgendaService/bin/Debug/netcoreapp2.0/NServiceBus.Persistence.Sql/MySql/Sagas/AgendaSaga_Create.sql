...
where table_schema = database() and
      column_name = 'Correlation_AgendaId' and
      table_name = @tableNameNonQuoted;

set @query = IF(
    @exist <= 0,
    concat('alter table ', @tableNameQuoted, ' add column Correlation_AgendaId varchar(38) character set ascii'), 'select \'Column Exists\' status');
...
set @query = IF(
    @column_type_AgendaId <> 'varchar(38) character set ascii',
    'call sqlpersistence_raiseerror(concat(\'Incorrect data type for Correlation_AgendaId. Expected varchar(38) character set ascii got \', @column_type_AgendaId, \'.\'));',
...
where
    table_schema = database() and
    index_name = 'Index_Correlation_AgendaId' and
    table_name = @tableNameNonQuoted;

set @query = IF(
    @exist <= 0,
    concat('create unique index Index_Correlation_AgendaId on ', @tableNameQuoted, '(Correlation_AgendaId)'), 'select \'Index Exists\' status');
